...
# start_modify: helper function for embedding with noise injection
def _compute_embeds_with_noise(
    ...
    Args:
        model: The model with get_input_embeddings() method (actor model from verl)
    ...
    Returns:
        List of embedding tensors (one per sample, without padding), converted to list format for SGLang
    """
    ...
    if special_mask.any():
        ...
        # Expand mask to hidden_dim: (batch_size, seq_len, 1) -> (batch_size, seq_len, hidden_dim)
        special_mask_expanded = special_mask.unsqueeze(-1).expand_as(inputs_embeds)
        inputs_embeds = inputs_embeds + noise * special_mask_expanded.float()
    
    # Convert to list format for SGLang (remove padding, convert to list)
    result = []
    for i in range(batch_size):
        ...
        # Convert to list of lists for SGLang input_embeds format
        result.append(embed_no_pad.cpu().tolist())
...
class SGLangRollout(BaseRollout):
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        input_embeds_list = None
        
        if use_input_embeds:
            # Check if input_embeds are pre-computed and passed via meta_info
            # (should be computed by the caller before switching to rollout mode)
            input_embeds_list = prompts.meta_info.get("input_embeds_with_noise", None)
            
            if input_embeds_list is None:
                logger.warning(
                    "use_input_embeds is True but input_embeds_with_noise not found in prompts.meta_info. "
                    "Make sure to compute embeddings before calling generate_sequences. "
                    "Falling back to input_ids mode."
                )
                use_input_embeds = False
            else:
                noise_std = self.config.get("noise_std", 1.0)
                special_token_id = self.config.get("special_token_id", None)
                logger.info(f"Using pre-computed input_embeds with noise_std={noise_std} for special_token_id={special_token_id}")
        # end_modify
