    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        if use_input_embeds and self._is_actor:
            ...
            if special_token_id is not None:
                # Compute embeddings while model is still on GPU (before offload)
                input_ids = prompts.batch["input_ids"]
                pad_token_id = meta_info["pad_token_id"]
                
                # For Megatron, get the module from actor_module list
                if hasattr(self, "actor_module") and len(self.actor_module) > 0:
                    model_for_embed = self.actor_module[0]
                    input_embeds_list = _compute_embeds_with_noise(
                        model=model_for_embed,
                        input_ids=input_ids,
                        special_token_id=special_token_id,
                        noise_std=noise_std,
                        pad_token_id=pad_token_id,
                    )
                    prompts.meta_info["input_embeds_with_noise"] = input_embeds_list
                    logger.info(f"Computed input_embeds with noise_std={noise_std} for special_token_id={special_token_id}")
        # end_modify
